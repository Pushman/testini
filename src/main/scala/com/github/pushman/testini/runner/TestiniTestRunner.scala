...
import com.github.pushman.testini.data.TestCase._
...
class TestiniTestRunner(clazz: Class[_]) extends BlockJUnit4ClassRunner(clazz) {
  ...
  private lazy val testMethodsKitsMap = testCases.toKitsMap

  private lazy val parameterisedTestRunner = new TestiniParameterisedTestRunner(testCases, descriptionProvider)

  override def computeTestMethods = testMethodsKitsMap.keys.toList

  override def getDescription: Description = descriptionProvider.describeTest(testCases)
